...
Abandon (core dumped)
        ...
        \pause
        \bigbreak
        \lstinline{stack smashing detected} veut dire que grâce à un mécanisme de protection fournit par GCC, l'OS a détecté un buffer overflow et fait crasher le programme de manière préventive.
...
Access Granted.
    ...
    \begin{frame}{Supply chain attack}{Exercice \execcounterdispinc}
        Développer en un nombre de lignes/caractères minimum, en JavaScript, les fonctions \lstinline{isOdd} et \lstinline{isEven}.
        \bigbreak
        Que pensez-vous de la balance bénéfice/risque de ces packages~?
        \bigbreak
        \centering
        \includegraphics[width=5cm]{image/5-odd-or-even}
    \end{frame}
    ...
    \begin{frame}{OAuth2}{L'application cliente ou client}
        ...
        Parmi elles Auth0 by Okta est un SAAS dédié aux services autour de OAuth2\footnote{Secure access for everyone But not just anyone, \url{https://auth0.com/}}.
        \bigbreak
    ...
    \begin{frame}{OAuth2}{Access Token\footnote{\label{auth0-tokens}ID Token and Access Token: What's the Difference~?, \url{https://auth0.com/blog/id-token-access-token-what-is-the-difference/}}}
        De base dans OAuth2, plusieurs formats de token sont possibles.
        \begin{itemize}
            \item \textbf{Utilisation}~: Autoriser l'accès aux ressources protégées (API, services).
            \item \textbf{Contenu}~: Informations sur les permissions et les ressources accessibles.
            \item \textbf{Audience}~: Destiné aux API et serveurs de ressources.
            \item \textbf{Durée de vie}~: Courte durée pour des raisons de sécurité.
        \end{itemize}
    \end{frame}

    \begin{frame}{OAuth2}{ID Token\cref{auth0-tokens}}
        Propre à OpenID Connect (OIDC) la couche d'identification au dessus de OAuth 2 et non à OAuth2.
        \bigbreak
        \begin{columns}
            \column{0.6\textwidth}
            \begin{itemize}
                \item \textbf{Utilisation}~: Authentifier l'utilisateur et fournir des informations sur son identité.
                \item \textbf{Contenu}~: Informations sur l'utilisateur (nom, email, etc.) et détails de l'authentification.
                \item \textbf{Audience}~: Destiné aux clients (applications) pour vérifier l'identité de l'utilisateur.
                \item \textbf{Durée de vie}~: Peut être plus longue que celle des access tokens.
            \end{itemize}
            \column{0.4\textwidth}
            \centering
            \includegraphics[width=5cm]{image/which-token}
        \end{columns}
    \end{frame}
    ...
    \begin{frame}{Implémentation d'un algorithme de hashage}{Pourquoi un mot de passe doit être hashé}
        \begin{itemize}
            \item Un mot de passe en clair est lisible si la base de données est compromise, c'est à proscrire.
            \item Le hashage est une fonction unidirectionnelle~: on ne peut pas retrouver le mot de passe original.
            \item Le chiffrement peut être inversé si la clé est connue, le hashage non.
            \item En cas de fuites de données, les informations pour régénérer le mot de passe ne sont dans la machine.
        \end{itemize}
    \end{frame}

    \begin{frame}{Implémentation d'un algorithme de hashage}{Implémentation d'un algorithme de hashage}{Recommandation sur les algorithme de hashage}
        \centering
        \includegraphics[width=8cm]{image/real-password-cracking} \\ Hashcat benchmark\footnotemark \\
    ...
    \begin{frame}{Implémentation d'un algorithme de hashage}{Qu'est-ce qu'un sel~?}
        \begin{itemize}
            \item Un \textbf{sel} est une valeur aléatoire ajoutée au mot de passe avant le hashage.
            \item Il rend chaque hashage unique, même pour deux mots de passe identiques.
            \item Il empêche les attaques par \textit{rainbow tables} (tables de hashage pré-calculées).
            \item Le sel est généralement stocké avec le hashage.
            \item Certains algorithmes comme Bcrypt ont, par défaut, le sel qui est formaté à l'intérieur du résultat du hash.
            Cela permet de ne pas le stocker dans la base (avoir un schéma légèrement plus simple) et surtout de faciliter son utilisation, car il est toujours là~!
        \end{itemize}
    \end{frame}

    \begin{frame}{Implémentation d'un algorithme de hashage}{Le nombre de rounds (itérations)}
        \begin{itemize}
            \item Le hashage doit être répété plusieurs fois pour prendre un temps quelques millisecondes~: ce sont les \textbf{rounds}.
            \item Plus il y a de rounds, plus le calcul est long.
            \item Cela ralentit les attaques par force brute ou dictionnaire.
            \item Des algorithmes comme Bcrypt, Scrypt ou Argon2 permettent de configurer ce paramètre.
        \end{itemize}
    \end{frame}

    \begin{frame}[fragile]{Implémentation d'un algorithme de hashage}{Exemple en Python avec Bcrypt du package \lstinline{bcrypt}}
        \begin{lstlisting}[language=Bash]
import bcrypt
# Step 1: Your password (must be bytes)
password = b"my_secure_password"
# Step 2: Define number of rounds (cost factor)
cost = 14  # You can adjust this (e.g., 10, 12, 14). Higher = more secure & slower
# Step 3: Generate salt with the cost factor
salt = bcrypt.gensalt(rounds=cost)
# Step 4: Hash the password with the generated salt
hashed = bcrypt.hashpw(password, salt)
# Step 5: Output the result
print("Hashed password (includes salt and cost):", hashed.decode())
# Step 6: Verifying later
if bcrypt.checkpw(password, hashed):
    print("✅ Password is correct.")
else:
    print("❌ Password is incorrect.")
Hashed password (includes salt and cost): $2b$14$xRoWpVV3QfZ6Re4WjA86DetuUvSMi5Gr7RFxTw3D2XD4/KlGsiJEW
✅ Password is correct.
        \end{lstlisting}
    \end{frame}

    \begin{frame}[fragile]{Implémentation d'un algorithme de hashage}{Exemple en Python avec PDKDF2}
        \begin{lstlisting}[language=Bash]
import secrets
import hashlib
import base64
...
hash_name = "sha256"
dklen = 32
# === Génération sécurisée du salt avec secrets ===
salt = secrets.token_bytes(salt_size)
# === Calcul du hash PBKDF2 ===
hash_bytes = hashlib.pbkdf2_hmac(hash_name, password, salt, iterations, dklen)
# === Encodage en base64 pour stockage
hash_b64 = base64.b64encode(hash_bytes).decode()
print("Hash stocké~:", hash_b64)
Hash stocké~: do8a86L5pd16mBu581z9Pa8lv0awIoI9Xowj591PdjM=
        \end{lstlisting}
    \end{frame}

    \begin{frame}[fragile]{Implémentation d'un algorithme de hashage}{Exemple en Python avec PDKDF2}
        \begin{lstlisting}[language=Bash]
def verify_password(input_password: bytes, stored_hash: str) -> bool:
    original_hash = base64.b64decode(hash_b64)
    # Recalcul du hash
    test_hash = hashlib.pbkdf2_hmac(hash_name, input_password, salt, iterations, dklen=len(original_hash))
    # Comparaison sécurisée
    return hmac.compare_digest(original_hash, test_hash)
# === Exemple d'utilisation ===
if verify_password(b"mon_mot_de_passe", hash_b64):
...
✅ Mot de passe valide
    ...
    \begin{frame}{TOTP et HOTP}
        \begin{itemize}
            ...
            \item HOTP est utilisé dans des applications où la synchronisation de l'horloge n'est pas possible, dans les envois de code par mail ou SMS.
            \item Le device (smartphone, tablette) qui génère le mot de passe peut être offline.
        \end{itemize}
    \end{frame}

    \begin{frame}{Adoption et popularité des applications de TOTP\footnote{10 Most Popular Two-Factor Authentication Apps Compared, \url{https://www.protectimus.com/blog/10-most-popular-2fa-apps-on-google-play/}}}
        \begin{tiny}
            \begin{tabular}{p{2cm} p{4.5cm} p{4.5cm}}
                \hline
                \textbf{Application}    & \textbf{Fonctionnalités clés}                                                                                                                                                                                                                                              & \textbf{Limites / Inconvénients}                                                                                                                              \\                                                                                                                                                       \\
                \hline
                Google Authenticator & Interface très simple, supporte TOTP et HOTP, fonctionne hors réseau, permet plusieurs comptes sur une seule appli. & Pas de sauvegarde intégrée, affichage limité à environ 4 comptes par écran.\\
                \hline
                Protectimus Smart OTP & Sauvegarde cloud chiffrée, transfert de tokens, import depuis Google Authenticator, authentification biométrique et PIN, support HOTP/TOTP/OCRA, code 6 ou 8 chiffres, organisation par dossiers et emojis, notifications push, signature de données (CWYS), multi-langue. & \\
                \hline
                Authy & Version desktop, sauvegarde cloud, PIN principal, synchronisation multi-appareils. & Certaines fonctionnalités via SMS (moins sûr), risque lié à la synchronisation multi-appareils, migration depuis une autre appli demande un smartphone rooté. \\
                \hline
                Microsoft Authenticator & Génération de codes / OAuth, notifications push \textquote{Verify/Deny}. & Problèmes signalés de fonctionnement et échange de données, pas de sauvegarde, affichage limité à environ 4 comptes à la fois. \\
                \hline
                FreeOTP Authenticator & Open-source, très léger (~500 KB), supporte HOTP et TOTP, compatible avec de nombreux services. & Interface peu ergonomique, pas de sauvegarde ni version desktop. \\
                \hline
                Sophos Authenticator & Supporte TOTP/HOTP, divers algorithmes (SHA-1/256/512), durée de code personnalisable, codes 6 ou 8 chiffres. & Pas de sauvegarde, problèmes signalés avec VPN et RADIUS. \\
                \hline
            \end{tabular}
        \end{tiny}
        \begin{footnotesize}
            \begin{dangercolorbox}
                Pas de données sur le nombre de téléchargements dans un des stores, donc à prendre avec des pincettes\ldots
            \end{dangercolorbox}
        \end{footnotesize}
    \end{frame}

    \begin{frame}{Adoption et popularité des applications de TOTP}
        Google / Microsoft Authenticator sont compatibles avec les \href{https://tools.ietf.org/html/rfc6238}{RFC 6238} pour TOTP et \href{https://tools.ietf.org/html/rfc4226}{RFC 4226} pour HOTP\footnote{Google Authenticator OpenSource, \url{https://github.com/google/google-authenticator}}.
        \bigbreak
        ...
        \begin{columns}
            \column{0.6\textwidth}
            \begin{itemize}
                \item Les applications mobiles, donc côté client, sont compatibles, interchangeables.
                Essayez avec \url{https://elogic.synology.me/TestAuthenticator/}
                \item Les packages côté backend le sont aussi~!
            \end{itemize}
            \column{0.4\textwidth}
            \centering
            \includegraphics[width=4cm]{image/protocols-satisfying}
        \end{columns}
    \end{frame}

    \begin{frame}{Exercice \execcounterdispinc{} sur TOTP}
        Ajouter au serveur déjà développé l'authentification TOTP compatible avec l'application mobile Google Authenticator.
        \bigbreak
        \begin{columns}
            \column{0.6\textwidth}
            ...
            \end{itemize}
            Les livrables sont le code et dans un document de synthèse, un diagramme de séquence UML qui contient~:
            \begin{itemize}
    ...
    \begin{frame}{Exercice \execcounterdispinc{} sur HOTP}
        À l'image de certaines applications dont la connexion est plus \textit{lean} sans mot de passe (\textit{subjectif...}) comme airbnb, AliExpress, qui se basent sur l'authentification du moyen de paiement et la sécurité de la boîte mail.
        Développez une application qui envoie un code HOTP par mail à la personne en cours de login.
        \bigbreak
        En Python, les librairies conseillées~:
        \begin{itemize}
            \item \href{https://pyauth.github.io/pyotp/}{PyOTP}
            \item \href{https://flask.palletsprojects.com/en/stable/}{Flask}
            \item Une API de mails transactionnels gratuite pour les développeurs ou étudiants.
            \item Stockez le compteur et l'adresse mail dans une base \href{SQL}{https://docs.python.org/3/library/sqlite3.html}.
        \end{itemize}
    \end{frame}
    ...
    \begin{frame}{Protection du serveur}{Sécurisation des clés et des secrets}
        ...
        \begin{itemize}
            ...
            \item Linux~: LUKS, \lstinline{ecryptfs-utils}, YubiKey, chiffrement du disque dès l'installation, \textit{etc}.
        \end{itemize}
    \end{frame}
...
server {
    ...
    \begin{frame}{Post-quantum cryptography}{Hype ou réél danger iminant~?}
        \begin{footnotesize}
            Exercice \execcounterdispinc{}, analyser l'article \href{https://www.earth.com/news/china-breaks-rsa-encryption-with-a-quantum-computer-threatening-global-data-security/}{China breaks RSA encryption with a quantum computer, threatening global data security}.
            \bigbreak
            RSA, peut-être simplifié comme suit~: $N$ est un semi-premier, c’est-à-dire que $p$ et $q$, ses facteurs, sont premiers.
            $N$ est la clé publique, $p$ et $q$ sont secrets.
            \begin{columns}

                \column{0.6\textwidth}
                \begin{itemize}
                    \item Quelle est la clé (en supposant que c'est le semi-premier le plus grans possible pour cette taille de clé)~?
                    \item Quels algorithmes peuvent trouver $p$ et $q$~?
                    \item Développer ces algorithmes dans le langage de votre choix.
                    Développer le plus facile sans I.A., il est très simple\ldots
                    \item Combien de temps cela prendrait-il avec un ordinateur classique~?
                \end{itemize}

                \column{0.4\textwidth}
                \begin{center}
                    \includegraphics[width=6cm]{image/rsa-both-ways}
                \end{center}

            \end{columns}

            \begin{itemize}
                \item A-t-on la durée du programme sur l'ordinateur quantique~?
                S’il est plus lent que l'ordinateur classique, il ne \textit{scalera} pas mieux.
                \item Quels sentiments transmet cet article, sont-ils renforcés par les résultats précédents~?
                \item Où en sont les ordinateurs classiques, ceux qui fonctionnent~?
            \end{itemize}
        \end{footnotesize}
    \end{frame}

    \begin{frame}{Logarithme discret}
        \begin{columns}
            \column{0.8\textwidth}
            \begin{center}
                \includegraphics[width=5cm]{image/dlog-both-ways}
            \end{center}
            \column{0.2\textwidth}
            $p$ est premier.
        \end{columns}
        \begin{footnotesize}
            Ce calcul, lui aussi déséquilibré est utilisé dans\footnote{Public key cryptography using discrete logarithms, \url{https://www.di-mgt.com.au/public-key-crypto-discrete-logs-0.html}}\footnotestep{}\footnote{EITCA, \url{https://fr.eitca.org/cybersecurity/eitc-is-acc-advanced-classical-cryptography/elliptic-curve-cryptography/introduction-to-elliptic-curves/examination-review-introduction-to-elliptic-curves/what-is-the-elliptic-curve-discrete-logarithm-problem-ecdlp-and-why-is-it-difficult-to-solve/}}~:
            \begin{itemize}
                \item Diffie-Hellman Key Exchange
                \item MQV Key Agreement, proche de Diffie-Hellman
                \item ElGamal Encryption
                \item Digital Signature Algorithm (DSA)
                \item ECDLP - Elliptic Curve Cryptography Discrete Logarithm Problem
            \end{itemize}
        \end{footnotesize}
    \end{frame}

    \begin{frame}{Logarithme classique vs.\ logarithme discret}
        \footnotesize
        \begin{columns}[T,totalwidth=\textwidth]
            \begin{column}{0.5\textwidth}
                \textbf{Logarithme classique (sur $\mathbb{R}$), permet de trouver $x$}.

                \[
                    h = g^{x} \;\Longrightarrow\; x = \log_{g}(h)
                \]
                ...
                \begin{itemize}
                    \item Fonctions continues et inversibles
                    \item Inverse efficace $\log_g$
                    \item Travail sur les nombres réels
                \end{itemize}
            \end{column}

            \begin{column}{0.5\textwidth}
                \textbf{Logarithme discret (modulaire), où $p$ est premier.}
                ...
                \textbf{Exemple~:}\,
                $5^{6} \equiv 8 \pmod{23}$.\,
                Ici $g=5$, $h=8$ ; on veut retrouver $x=6$.

                \bigbreak
                \textbf{Pourquoi c'est difficile~:}
                \begin{itemize}
                    \item Les exposants bouclent dans un groupe fini.
                    Ils sont aussi périodiques.
                    \item Pas de fonction inverse connue efficace en général
                    \item Base de sécurité de Diffie-Hellman, ElGamal, ECC (ECDLP), DSA
                \end{itemize}
            \end{column}
        \end{columns}
    \end{frame}

    \begin{frame}{Post-quantum cryptography}{À risque ou pas~?}
        \small
        Exercice \execcounterdispinc{}~:
        \begin{itemize}
            \item Pour $p=23$ et $g=5$, calculer les puissances successives de $g$ modulo $p$ avec Python, que remarquez-vous~?
            \item Quel lien avec le précédent article~?
        \end{itemize}
        \bigbreak
        \begin{center}
            \includegraphics[width=5cm]{image/quantum-computer}
        \end{center}
    \end{frame}

    \begin{frame}{Post-quantum cryptography}{La multiplication scalaire à risque}

        \begin{columns}
            \column{0.5\textwidth}
            La multiplication scalaire est utilisée dans la plupart des algorithmes de cryptographie à courbe elliptique (ECC).
            \column{0.5\textwidth}
            \begin{center}
                \includegraphics[width=6cm]{image/secp256k1}
            \end{center}
        \end{columns}

        \begin{itemize}
            \item Ethereum et Bitcoin utilisent la courbe secp256k1.
            \item ED25519 est une autre courbe très utilisée.
            SSH, Monero, TLS, \textit{etc}.
            \item X25519 est utilisée pour l'échange de clés dans le protocole TLS 1.3, WhatsApp, Signal, \textit{etc}.
            \item \textit{Many more}
        \end{itemize}

        \bigbreak
        \begin{dangercolorbox}
            Cette dernière est périodique et est donc vulnérable aux attaques par l'algorithme de Shor.
        \end{dangercolorbox}
    \end{frame}
